...
كل الأوامر داخل هذا الملف (بدون حاجة لمجلد handlers/)
"""
import asyncio
import logging
import sys
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        tier_name = {"free": "🆓 مجاني", "silver": "🥈 فضي", "gold": "🥇 ذهبي", "diamond": "💎 ماسي", "admin": "👑 مدير"}.get(user.tier, "🆓 مجاني")

        msg = f"""🐆 *أهلاً بك في الفهد!*
...
async def cmd_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if direction not in ("buy", "sell", "شراء", "بيع"):
        ...
        return
    if direction in ("شراء",): direction = "buy"
    elif direction in ("بيع",): direction = "sell"
...
"""
            if risk:
                resp += f"""🛑 وقف الخسارة: {risk.get('sl_pct', 5)}%
...
async def cmd_live(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "status":
        if engine.has_live_trading(user_id):
            ...
            await update.message.reply_text("🎮 *وضع افتراضي*\n\nللربط: `/live connect okx KEY SECRET`")
        return
    ...
    if action == "connect":
        ...
        try:
            success = await engine.connect_exchange(user_id, ex_name, api_key, api_secret, passphrase)
            if success:
...
async def cmd_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "add" and len(args) >= 3:
        ...
        tier_name = {"free": "مجاني", "silver": "فضي", "gold": "ذهبي", "diamond": "ماسي", "admin": "مدير"}.get(tier, tier)
...
async def cmd_autotrade(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action in ("on", "تشغيل"):
        ...
        await update.message.reply_text("🤖 *التداول الآلي*\nالحالة: ❌ موقف\nللتفعيل: `/autotrade on`")
