...
Bitget Skill: write-safety, demo-mode guards
"""
import re
import logging
...
# === Write-Safety Keywords (Bitget Skill pattern) ===
WRITE_ACTIONS = {"buy", "sell", "connect", "reset", "trigger", "transfer", "withdraw"}
...
class Guard:
    ...
    def is_write_action(action: str) -> bool:
        """Bitget Skill: تحديد إذا كان الإجراء يتطلب تأكيد"""
        return action.lower() in WRITE_ACTIONS

    @staticmethod
    def require_confirmation(action: str, is_demo: bool = False) -> str:
        """Bitget Skill: إنشاء رسالة تأكيد قبل الكتابة"""
        prefix = "🧪 [DEMO] " if is_demo else "⚠️ [CAUTION] "
        return f"{prefix}أنت على وشك: {action}\n\nاكتب 'yes' للتأكيد أو 'no' للإلغاء."
