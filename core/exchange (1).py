...
FIXED: Exception handling, graceful disconnect
"""
import logging
...
class ExchangeAdapter:
    ...
    async def disconnect(self):
        """فصل الاتصال — FIXED: مع معالجة الأخطاء"""
        if self._client:
            try:
                await self._client.close()
                logger.info(f"🔌 Disconnected from {self.exchange_id.upper()}")
            except Exception as e:
                logger.warning(f"Disconnect warning for {self.exchange_id}: {e}")
            finally:
                self._client = None
...
class ExchangeManager:
    ...
    async def connect_user(self, user_id: int, exchange_id: str,
        ...
        if user_id in self._connections:
            await self._connections[user_id]["adapter"].disconnect()
    ...
    async def disconnect_user(self, user_id: int):
        """فصل مستخدم — FIXED: async with error handling"""
        if user_id in self._connections:
            await self._connections[user_id]["adapter"].disconnect()
