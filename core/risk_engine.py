...
class RiskEngine:
    ...
    def assess(self, user_id: int, symbol: str, direction: str,
        ...
        if new_exposure > max_exposure:
            ...
            size_usd = allowed
            reasons.append(f"📉 التعرض قُلّص بسبب حد الباقة")
        ...
        # 5. ATR-based SL/TP
        sl_pct = max(atr_pct * 1.5, 3.0)   # SL = 1.5x ATR (min 3%)
        tp_pct = max(atr_pct * 2.5, 5.0)   # TP = 2.5x ATR (min 5%)

        # 6. Market Sentiment Adjustment
        if market_sentiment == "extreme_fear":
            sl_pct *= 1.2  # SL أوسع في الخوف
            tp_pct *= 1.3  # TP أبعد (فرصة)
        elif market_sentiment == "extreme_greed":
            sl_pct *= 0.9  # SL أضيق
            tp_pct *= 0.8  # TP أقرب
            size_usd *= 0.8  # حجم أصغر
